...
├── docker/
│   └── mysql/
│       └── 02_inicializacion.sh  carga G3_complementos.sql y otorga permisos
...
La carga inicial usa el mecanismo de MySQL: todo lo que se monte en `/docker-entrypoint-initdb.d` se ejecuta en orden alfabético la primera vez que se crea el volumen. El orden importa, porque `G3_complementos.sql` tiene una llave foránea hacia `usuario` y fallaría si corriera antes del modelo. Por eso el compose monta:

1. `database/script.sql` como `01_script.sql`, que crea la base, las tablas y los datos.
2. `docker/mysql/02_inicializacion.sh`, que carga `database/G3_complementos.sql` dentro de la base y le otorga permisos al usuario de la aplicación.
...
mysql -u root -p bdiedlavictoria < database/G3_complementos.sql
```
